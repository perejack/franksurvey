...
import { Clock, Star, ChevronRight, Lock, CheckCircle2, Sparkles } from "lucide-react";
...
const CATEGORY_IMAGES: Record<string, string> = {
  ...
  education: catLifestyle,
  sports: catEntertainment,
};
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center flex-shrink-0 overflow-hidden">
          {catImage ? (
            <img src={catImage} alt={survey.category} className="w-full h-full object-cover" />
          ) : (
            <span className="text-2xl">{categoryIcons[survey.category] || "📋"}</span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {isNewlyUnlocked && (
              <span className="text-[10px] font-bold uppercase tracking-wider bg-green-500 text-white px-2 py-0.5 rounded-full flex items-center gap-1">
